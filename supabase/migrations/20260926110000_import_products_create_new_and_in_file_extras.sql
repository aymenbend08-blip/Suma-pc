-- import_products(): two parity fixes with SUMA Web's importer.
--
-- 1) Duplicate strategy 'create_new' (SUMA Web: "أنشئ منتج جديد لكل واحد
--    فيهم"): a row that matches an existing product is created as a NEW
--    product anyway. A code can only ever point at one product per store,
--    so the matched barcode is left on the existing product and the new
--    one is created without a main barcode (and without its internal code
--    when that code is taken), with a warning telling the owner to fix it
--    — exactly SUMA Web's bulkImportProducts behaviour. Admin-only, like
--    every product creation (products_admin_insert).
--
-- 2) In-file barcode tracking now covers EXTRA barcodes too (SUMA Web's
--    classifyRow does): a main barcode already used as an extra barcode by
--    an earlier row of the same file is an error for that row, and an extra
--    barcode already used by an earlier row is dropped with a warning —
--    in the dry run as well, so the preview matches the real import.
--
-- Everything else is the 20260926104000 body with the 20260926105000
-- name-match patch folded in. Same signature; CREATE OR REPLACE in place.
-- 'update' | 'skip' | 'barcode_only' keep their exact behaviour.
...
BEGIN
  ...
  END IF;
  IF _duplicate_strategy NOT IN ('update', 'skip', 'barcode_only', 'create_new') THEN
    ...
    BEGIN
      ...
      IF jsonb_typeof(_row->'extra_barcodes') = 'array' THEN
        FOR _raw_extra IN SELECT value FROM jsonb_array_elements(_row->'extra_barcodes') LOOP
          ...
          IF _extra = ANY(_seen) THEN
            _warnings := _warnings || jsonb_build_object('row', _line,
              'note', 'الباركود الإضافي ' || _extra || ' مستعمل في سطر سابق من نفس الملف — تم تجاهله.');
            CONTINUE;
          END IF;
      ...
      -- ---------- category
      _cat_id := NULL;
      IF _cat_name IS NOT NULL AND (_match.id IS NULL OR _duplicate_strategy = 'create_new' OR (_duplicate_strategy = 'update' AND _matched_by <> 'variant_barcode')) THEN
      ...
      IF _match.id IS NULL OR _duplicate_strategy = 'create_new' THEN
        -- ---------- new product (also 'create_new' on a match)
        IF NOT _is_admin THEN
          RAISE EXCEPTION 'إضافة منتجات جديدة محجوزة لصاحب المحل والمدير.' USING ERRCODE = '42501';
        END IF;
        _new_barcode := _barcode;
        _new_internal := _internal;
        IF _match.id IS NOT NULL THEN
          -- SUMA Web's "create_new": a code may only ever point at ONE
          -- product, so whichever code matched stays with the existing
          -- product and the new row is created without it — flagged for
          -- the owner to fix by hand.
          IF _matched_by IN ('barcode', 'extra_barcode', 'variant_barcode') THEN
            _new_barcode := NULL;
            _warnings := _warnings || jsonb_build_object('row', _line,
              'note', _name || ': تزاد كمنتج جديد بلا باركود أساسي (الباركود ' || _barcode || ' محجوز بالمنتج «' || _match.name || '») — راجعو.');
          END IF;
          IF _new_internal IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.products WHERE store_id = _store_id AND internal_code = _new_internal
          ) THEN
            _new_internal := NULL;
            _warnings := _warnings || jsonb_build_object('row', _line,
              'note', _name || ': تزاد بلا كود داخلي (الكود ' || _internal || ' مستعمل لمنتج آخر) — راجعو.');
          END IF;
          IF _matched_by = 'name' THEN
            _warnings := _warnings || jsonb_build_object('row', _line,
              'note', _name || ': تزاد كمنتج جديد رغم وجود منتج بنفس الاسم.');
          END IF;
        END IF;
        IF NOT _dry_run THEN
          ...
          VALUES
            (_store_id, _name, _new_barcode, _new_internal, _selling, _purchase, COALESCE(_stock, 0), COALESCE(_unit, 'وحدة'),
        ...
        IF _duplicate_strategy = 'update' THEN
          ...
          IF NOT _dry_run THEN
            UPDATE public.products SET
              name = CASE WHEN _matched_by = 'name' THEN name ELSE _name END,
              barcode = CASE
